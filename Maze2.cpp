...
if(Check_config_file()){//如果配置文件存在
mazesimulate->Log_class.cout_Log("The config file already exists - > Trying to delete the file...", Log_Exit::Log_type::NOTICE_ENUM);//输出通知日志
    if(std::remove(configPath.c_str())){//删除失败
        ...
        mazesimulate->Log_class.cout_Log("Deleted " + configPath + " files", Log_Exit::Log_type::NOTICE_ENUM);//输出通知日志
...
nlohmann::json configJson;
    ...
    if (out.is_open()) {
        ...
        mazesimulate->Log_class.cout_Log("Created " + configPath + " files", Log_Exit::Log_type::NOTICE_ENUM);//输出通知日志
...
mazesimulate->Log_class.cout_Log("No matching table found. - > Cancel creating JSON", Log_Exit::Log_type::NOTICE_ENUM);//输出通知日志
...
if(inFile.is_open()){
    ...
    mazesimulate->Log_class.cout_Log("Unable to open file " + configPath + " - > Launch using default configuration" , Log_Exit::Log_type::WARNING_ENUM);
}
...
void Mazesimulate::Log_Exit::cout_Log(std::string log, Log_type type){//输出日志信息并记录日志信息
...
void Mazesimulate::Input_processing::Get_Input(){
            ...
            if(token == "-help"){//如果输入是-help代表显示帮助信息
                ...
                std::cout << "Config Parameter Table:" << std::endl;
                    if(config.empty()){
                        ...
                        std::cout << config.config_name << ":" << config.config_description << " - > " << *config.Address <<std::endl;
                        }
    ...
    for(const auto& input : inputs){//处理输入内容
        ...
        mazesimulate->Log_class.cout_Log(Temporary_log.str(), Log_Exit::Log_type::PROMPT_ENUM);//输出提示日志
    }
...
mazesimulate->Log_class.cout_Log(Temporary_log.str(), Log_Exit::Log_type::WARNING_ENUM);//输出错误日志
