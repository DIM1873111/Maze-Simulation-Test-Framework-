...
void Two_way_group_adim__search(Maze_AI& maze){//Bionic algorithm(A)
    //贪心最佳优先的框架 + Dijkstra的距离记录 + 简单的反哺机制 + 蚂蚁算法的启发式信息素(在这里是g) + 双向搜索
    //G代替了多个作用
    //实际上双向的效率提升非常大，尤其是在大规模地图上
    //算法特点：不同于任何一个传统算法 本算法速度更快 可以保证几乎找到最优路线(实际调整完完全全可以到达最优(100%))正常使用大概在(99%~99.9%)之间
    //经过实际测试(包括一些极端情况) 只要调整好提前停止乘数 基本上都能达到99%以上的最优率(甚至在一些情况下可以达到100%) 当然如果不调整提前停止乘数 那么可能会有较大的偏差(甚至在一些极端情况下可能会有较大的偏差) 所以建议根据实际情况调整提前停止乘数(一般来说1.5~2.5之间比较合适) 
    //对比传统的A*算法 双向的效率提升非常大(内存占用,搜索速度,正确率,自适应能力(可以适用非网格迷宫 但是需要调整)) 甚至在一些情况下 可能会有数几倍的效率提升 当然这也取决于地图的规模和复杂度
    //除此之外 双向可以提供一个校准机制 通过比较两条路线的长度 可以判断是否需要继续搜索 (正常使用1可以达到97%最短路线 排除和实际最短路线1~3大概)
    //本算法 把多种算法思想巧妙的紧密结合到一起 在这里 A*的公式(f=g+h)在这里是完完全全不一样的作用
    //Wilson_algorithm生成的迷宫太过于复杂 这个算法明显不适合 主要是双向的提升不明显 因为迷宫对称复杂 在这里推荐使用其他算法 比如(标准A*)
    //为了大部分特定的迷宫制作的算法 理论上适配大部分迷宫 除了对称性复杂的 迷宫(比如Wilson_algorithm生成的迷宫) 
    //算法[20260531]
struct TotalCost_Struct{
int Number;//移动编号(偏移量)
int f_Value;//总代价
...
int OffsetX[4] = {-1,0,0,1};//x偏移量
int OffsetY[4] = {0,-1,1,0};//y偏移量
/*默认1,0就是另一个找到终点直接停止 可以根据自己需要调整乘数*/
int Early_stop_multiplier = 1;//提前停止乘数(当两条路线的长度之和小于最快路径长度*提前停止乘数时就提前停止搜索)

int Step_count_a[Ymax][Xmax];//行走距离
int Step_count_b[Ymax][Xmax];//行走距离
std::fill(Step_count_a[0], Step_count_a[0] + Ymax * Xmax, -1);//初始化行走距离
std::fill(Step_count_b[0], Step_count_b[0] + Ymax * Xmax, -1);//初始化行走距离
Step_count_a[Starting_point_Y][Starting_point_X] = 0;//路线起点行走距离为0(起点)
Step_count_b[destination_Y][destination_X] = 0;//路线起点行走距离为0(终点)

bool Search_ended = false;//搜索是否结束(false/true)

int Fastest_route_Length = Xmax * Ymax;//最快路径长度
std::vector<std::pair<int, int>> Fastest_route_vector;//最快路径

while(!Search_ended){

Search_ended = true;//提前设置搜索结束(如果没有找到更优路径就结束)
bool Found_you_a = false;//是否找到了终点(false/true)(a)
bool Found_you_b = false;//是否找到了起点(false/true)(b)

int Task_X_a = Starting_point_X;//导入起点(起点)
int Task_Y_a = Starting_point_Y;

int Task_X_b = destination_X;//导入终点(终点)
int Task_Y_b = destination_Y;

std::vector<std::pair<int, int>> Historical_line_vector_a;//历史路径统计数据(a)
std::vector<std::pair<int, int>> Historical_line_vector_b;//历史路径统计数据(b)

while(!(Found_you_a && Found_you_b)){

    if(!Found_you_a){//A路线
    TotalCost_Struct Highest_value = {-1,INT_MAX};

    for(int i = 0;i<4;i++){
    int Search_X = Task_X_a + OffsetX[i];
    int Search_Y = Task_Y_a + OffsetY[i];
    if(!maze.Boundary_check(Search_X,Search_Y) || maze.Tag_Information(Search_X,Search_Y,maze.Maze_walls_Enum)){continue;}//超过范围以及墙壁跳过本次循环}
    int h = abs(destination_X - Search_X) + abs(destination_Y - Search_Y);//计算估测距离
    int g = Step_count_a[Search_Y][Search_X]+1;//实际行走
    int f = g + h;//总代价
    if(f <= Highest_value.f_Value){
      Highest_value = {i,f};
        }
    }
    if(Highest_value.Number == -1){maze.exits("No path found",false);}//没有可选方向了

    int Search_X = Task_X_a + OffsetX[Highest_value.Number];
    int Search_Y = Task_Y_a + OffsetY[Highest_value.Number];
    if(maze.Check_Destination(Search_X,Search_Y)){Found_you_a=true;}    



    Step_count_a[Search_Y][Search_X] = Step_count_a[Task_Y_a][Task_X_a] + 1;//实际行走

    auto it = std::find(Historical_line_vector_a.begin(), Historical_line_vector_a.end(), std::make_pair(Search_X,Search_Y));
    std::pair<int,int> Next_Pos = std::make_pair(Search_X, Search_Y);//即将行走的位置

    if(it != Historical_line_vector_a.end()){//如果这个位置在历史路径中出现过
    Historical_line_vector_a.erase(it + 1, Historical_line_vector_a.end());//擦除这个位置之后的历史路径(回退)
    }else{
    if(Historical_line_vector_a.empty()){
        Historical_line_vector_a.push_back({Starting_point_X,Starting_point_Y});//导入起点
    }
    Historical_line_vector_a.push_back(Next_Pos);//加入历史路径统计数据
    maze.Mark_Trace(Search_X,Search_Y,maze.Map_Scan_Focus_Enum);//标记    
    }

    Task_X_a = Search_X;
    Task_Y_a = Search_Y;//移动到下一个位置
    }
    ...
    for(int i = 0;i<4;i++){
    int Search_X = Task_X_b + OffsetX[i];
    int Search_Y = Task_Y_b + OffsetY[i];
    if(!maze.Boundary_check(Search_X,Search_Y) || maze.Tag_Information(Search_X,Search_Y,maze.Maze_walls_Enum)){continue;}//超过范围以及墙壁跳过本次循环}
    int h = abs(Starting_point_X - Search_X) + abs(Starting_point_Y - Search_Y);//计算估测距离
    int g = Step_count_b[Search_Y][Search_X]+1;//实际行走
    int f = g + h;//总代价
    if(f <= Highest_value.f_Value){
      Highest_value = {i,f};
        }
    ...
    int Search_X = Task_X_b + OffsetX[Highest_value.Number];
    int Search_Y = Task_Y_b + OffsetY[Highest_value.Number];
    if(maze.Check_starting_point(Search_X,Search_Y)){Found_you_b=true;}    



    Step_count_b[Search_Y][Search_X] = Step_count_b[Task_Y_b][Task_X_b] + 1;//实际行走
    auto it = std::find(Historical_line_vector_b.begin(), Historical_line_vector_b.end(), std::make_pair(Search_X,Search_Y));
    std::pair<int,int> Next_Pos = std::make_pair(Search_X, Search_Y);//即将行走的位置

    if(it != Historical_line_vector_b.end()){//如果这个位置在历史路径中出现过
    Historical_line_vector_b.erase(it + 1, Historical_line_vector_b.end());
    }else{
    if(Historical_line_vector_b.empty()){
        Historical_line_vector_b.push_back({destination_X,destination_Y});//导入终点
    }
    Historical_line_vector_b.push_back(Next_Pos);//加入历史路径统计数据
    maze.Mark_Trace(Search_X,Search_Y,maze.Map_Scan_Focus_Enum);//标记    
    }

    Task_X_b = Search_X;
    Task_Y_b = Search_Y;//移动到下一个位置
    }


    if(Found_you_a){
    for(int i = 0;i<Historical_line_vector_a.size();i++){
        int x = Historical_line_vector_a[i].first;
        int y = Historical_line_vector_a[i].second;
        Step_count_a[y][x] = Historical_line_vector_a.size();//设置行走距离
        maze.Mark_Trace(x,y,maze.Map_Search_Focus_Enum);//标记
            }
    }  

    if(Found_you_b){
    for(int i = 0;i<Historical_line_vector_b.size();i++){
        int x = Historical_line_vector_b[i].first;
        int y = Historical_line_vector_b[i].second;
        Step_count_b[y][x] = Historical_line_vector_b.size();//设置行走距离
        maze.Mark_Trace(x,y,maze.Map_Search_Focus_Enum);//标记
            }
    }


    if(Fastest_route_Length>Historical_line_vector_a.size() && Found_you_a){
    Fastest_route_vector = Historical_line_vector_a;//更新最快路径统计数据
    Fastest_route_Length = Fastest_route_vector.size();//更新最快路径长度
    Search_ended = false;//找到更优路径，继续搜索
    }

    if(Fastest_route_Length>Historical_line_vector_b.size() && Found_you_b){
    Fastest_route_vector = Historical_line_vector_b;//更新最快路径统计数据
    Fastest_route_Length = Fastest_route_vector.size();//更新最快路径长度
    Search_ended = false;//找到更优路径，继续搜索
    }

    if(Historical_line_vector_a.size()>Fastest_route_Length*Early_stop_multiplier && !Found_you_a){//强制停止A
    Found_you_a = true;
    }

    if(Historical_line_vector_b.size()>Fastest_route_Length*Early_stop_multiplier && !Found_you_b){//强制停止B
    Found_you_b = true;
    }
    ...
    //刷新地图
    std::stringstream Current_content;
    Current_content << "Processing[" << Task_X_a << "," << Task_Y_a << "|" << Step_count_a[Task_Y_a][Task_X_a] << "|-|" << Task_X_b << "," << Task_Y_b << "|" << Step_count_b[Task_Y_b][Task_X_b] <<"]";
    maze.Map_loading(Current_content.str());
...
void Genetic_algorithm(Maze_AI& maze){//遗传算法
int Xmax = maze.X_Map_Max(),Ymax = maze.Y_Map_Max();

int Uninitialized = -1;//未初始化标记
...
int Gene_length = (Xmax + Ymax) * 2;//基因长度
int End_search = (Xmax + Ymax) / 2;//结束搜索
struct Individual{//个体结构体
    std::vector<int> Gene;//基因
    int Fitness = 1;//适应度
};

struct Statistics{//统计基因价值结构体
    int Probability_Start;//选择概率开始的范围
    int Probability_End;//选择概率结束的范围
    int i;//个体编号
};
...
int Population_Quantity = 50;//种群数量


for(int i = 0;i<Population_Quantity;i++){//初始化种群
    std::vector<int> Individual;//个体
    for(int j = 0;j<Gene_length;j++){
        Individual.push_back(maze.Random_number(0,3));//随机生成基因(0-3分别代表四个方向)
    }
    Population.push_back({Individual, 0});
}
int Findthe_end_point = 0;//找到终点的次数
while(Findthe_end_point < End_search){//主循环

std::vector<Statistics> Statistics_vector;//统计基因价值
int Probability_starts = 0;//概率盘开始

for(int i = 0;i<Population_Quantity;i++){//计算每一个基因的适应度
int Task_X = Starting_point_X;//导入起点
int Task_Y = Starting_point_Y;

    for(int j = 0;j < Population[i].Gene.size();j++){//计算单个基因的适应度
        int Move = Population[i].Gene[j];//移动
        int Search_X = Task_X + OffsetX[Move];//搜索位置
        int Search_Y = Task_Y + OffsetY[Move];//搜索位置
        if(!maze.Boundary_check(Search_X,Search_Y) || maze.Tag_Information(Search_X,Search_Y,maze.Maze_walls_Enum)){break;}//超过范围以及墙壁跳过本次循环}
        if(maze.Tag_Information(Search_X,Search_Y,maze.Map_Scan_Focus_Enum)){continue;}//已经走过的跳过本次循环
        if(maze.Check_Destination(Search_X,Search_Y)){//找到终点
            Population[i].Fitness += Gene_length + 1 - j;//适应度根据距离增加
            Findthe_end_point++;//找到终点次数+1
            break;
        }
        
        Task_X = Search_X;//移动到下一个位置
        Task_Y = Search_Y;
        Population[i].Fitness++;//适应度+1(每移动一步适应度+1)
        //地图更新
        maze.Mark_Trace(Task_X,Task_Y,maze.Map_Scan_Focus_Enum);//标记(Map_Scan_Focus_Enum=Scan_Mark)

    }

    if(Findthe_end_point == End_search){//结束搜索
    for(int i = 0;i<Ymax;i++){
        for(int j = 0;j<Xmax;j++){
            if(maze.Tag_Information(j,i,maze.Map_Scan_Focus_Enum)){//标记最快路径
                maze.Mark_Trace(j,i,maze.Maze_Shortest_route);//标记最快路径
                }
            }
        }
        std::stringstream Current_content;//刷新地图
        Current_content << "Shortest path...[" << Task_X << "," << Task_Y << "]";
        maze.Map_loading(Current_content.str());          
    break;
    
    }else{
        std::stringstream Current_content;
        Current_content << "Processing[" << Task_X << "," << Task_Y << "|" << Population[i].Fitness << "|-|" << Task_X << "," << Task_Y << "|" << Population[i].Fitness <<"]";
        maze.Map_loading(Current_content.str());         
    }


    
    int Probability_End = Probability_starts + Population[i].Fitness;//选择概率结束的范围
    Statistics_vector.push_back({Probability_starts, Probability_End, i});//记录统计数据
    Probability_starts = Probability_End + 1;//更新选择概率开始的范围


}
    if(Findthe_end_point == End_search){continue;}//结束搜索

    std::vector<Individual> New_population;//新种群
    //组合(在概率里找两个进行组合 变异作为下一个)

    for(int i = 0;i < Population_Quantity;i++){

    int Father = Uninitialized , Mother = Uninitialized;//父母编号
    int Father_Choose = Uninitialized , Mother_Choose = Uninitialized;
    while(Father_Choose == Mother_Choose){//相同重新选择
    Mother_Choose = maze.Random_number(0, Probability_starts - 1);
    Father_Choose = maze.Random_number(0, Probability_starts - 1);
    }
    for(int i = 0;i < Population_Quantity;i++){
        if(Mother_Choose>=Statistics_vector[i].Probability_Start && Mother_Choose<=Statistics_vector[i].Probability_End){
            Mother = Statistics_vector[i].i;//选择了这个个体作为母亲
        }
        if(Father_Choose>=Statistics_vector[i].Probability_Start && Father_Choose<=Statistics_vector[i].Probability_End){
            Father = Statistics_vector[i].i;//选择了这个个体作为父亲
        }
    }

    int Cut_position = maze.Random_number(0, Gene_length - 1);//交叉位置

    Individual Combination_gene;//交叉后的基因
    Combination_gene.Gene.reserve(Gene_length);//预留空间
    for(int i = 0;i < Gene_length;i++){//交叉
        if(i>=Cut_position){
            Combination_gene.Gene.push_back(Population[Father].Gene[i]);
        }else{
            Combination_gene.Gene.push_back(Population[Mother].Gene[i]);
        }
    }
    ...
    //变异_随机改变一个基因(可能)载入(随机改变一个基因)

    
    Individual About_to_load = Combination_gene;//载入_基因 (可能变异)
    if(maze.Random_number(0, 10) > 3){//变异 (概率)
        int Mutation_position = maze.Random_number(0, Gene_length - 1);//变异位置
        About_to_load.Gene[Mutation_position] = maze.Random_number(0, 3);//变异
        }
        New_population.push_back(About_to_load);//载入
    }
